...
- 常時読む入口から本文までの経路を「入口 → 必要なら索引 → 詳細文書」の順で確認します。
- 条件付き文書には、依頼条件と正確な文書パスを対応させた明示的な流入参照を少なくとも
  1つ持たせます。
- 参照先Markdownの自動読込や、「関連文書」「該当文書」のような抽象的な案内だけに
  依存しません。
- 対象のルートテンプレート、導入互換性チェック、セットアップ検証、
  `scripts/validate_repository.py`の必須ルーティングを同期します。
- `rg`で詳細文書名の流入参照を確認し、索引から到達できない指示文書を残しません。
...
- 条件付き指示へ、依頼条件と正確なパスを持つ入口または索引から到達できる。
